...
import type { SchemaMapping, FieldMapping } from "../types/index.js";
...
Examples:
  ...
  - Use when: "Store the user profile mapping for later use"`,
    ...
    async (params) => {
      try {
        const fieldMappings: FieldMapping[] = JSON.parse(params.field_mappings);
    ...
    async ({ mapping_id, response_format }) => {
      try {
        ...
        return {
          ...
          structuredContent: mapping as unknown as Record<string, unknown>
        };
      } catch (error) {
        const errorMsg = error instanceof Error ? error.message : String(error);
        return {
