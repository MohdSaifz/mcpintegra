...
export class MappingService {
     ...
     */
    static suggestMappings(sourceSchema, targetSchema, confidenceThreshold = DEFAULT_CONFIDENCE_THRESHOLD) {
        const suggestions = [];
        const sourceFields = this.extractFields(sourceSchema);
        const targetFields = this.extractFields(targetSchema);
        for (const sourceField of sourceFields) {
            for (const targetField of targetFields) {
                const confidence = this.calculateFieldSimilarity(sourceField.path, targetField.path, sourceField.schema, targetField.schema);
                if (confidence >= confidenceThreshold) {
                    const transformation = this.suggestTransformation(sourceField.schema, targetField.schema);
                    suggestions.push({
                        confidence,
     ...
     */
    static calculateFieldSimilarity(sourceName, targetName, sourceSchema, targetSchema) {
        let score = 0;
        // Exact match
     ...
     */
    static checkSynonyms(name1, name2) {
        for (const synonyms of Object.values(FIELD_SYNONYMS)) {
            const normalizedSynonyms = synonyms.map(s => this.normalizeName(s));
     ...
     */
    static calculateTypeCompatibility(type1, type2) {
        if (type1 === type2)
            return 1.0;
        // Compatible types
     ...
     */
    static suggestTransformation(sourceSchema, targetSchema) {
     ...
     */
    static generateReasoning(sourceField, targetField, confidence) {
        const reasons = [];
     ...
     */
    static extractFields(schema, prefix = "") {
        const fields = [];
     ...
     */
    static createFieldMapping(suggestion) {
        return {
...
//# sourceMappingURL=mappingService.js.map